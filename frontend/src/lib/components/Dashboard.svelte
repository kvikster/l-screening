...
<div class="mx-auto max-w-7xl space-y-8 p-8">
  <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
    <div>
      <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">LC-MS Screening</p>
      <h2 class="mt-2 text-3xl font-bold tracking-tight text-slate-950 dark:text-slate-50">{title}</h2>
    </div>
    <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
      <span class="font-semibold text-slate-900 dark:text-slate-100">Threshold profile:</span>
      ...
      {#if metadata?.cacheHit}
        <span class="ml-2 rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">cache hit</span>
      {/if}
    </div>
  </div>
  ...
  <div class="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div class="border-b border-slate-100 px-6 py-5 dark:border-slate-700">
        <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">QC Summary</h3>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Per sample type and polarity: replicate confirmation, quality profile, confidence and blank behaviour.</p>
      </div>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
            {#each summary as row}
              <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-700/50">
                <td class="px-4 py-3 font-semibold capitalize text-slate-900 dark:text-slate-100">{row.Sample}</td>
                <td class="px-4 py-3 uppercase text-slate-500 dark:text-slate-400">{row.Polarity}</td>
                <td class="px-4 py-3 dark:text-slate-300">{row.Confirmed}</td>
                <td class="px-4 py-3 dark:text-slate-300">{row.RealCompounds || 0} / {row.Artifacts || 0}</td>
                <td class="px-4 py-3 font-mono dark:text-slate-300">{formatMaybe(row.MeanCVPct, 1)}</td>
                <td class="px-4 py-3 dark:text-slate-300">{row.HighQuality || 0}/{row.ModerateQuality || 0}/{row.LowQuality || 0}</td>
                <td class="px-4 py-3 font-mono dark:text-slate-300">{formatMaybe(row.MeanConfidenceScore, 1)}</td>
                <td class="px-4 py-3 font-mono dark:text-slate-300">{formatMaybe(row.MeanSignalToBlankRatio, 2)}</td>
              </tr>
            {/each}
          </tbody>
        </table>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div class="border-b border-slate-100 px-6 py-5 dark:border-slate-700">
        <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Method Parameters</h3>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">The exact screening thresholds applied to this result set.</p>
      </div>
      <dl class="grid gap-3 px-6 py-5 text-sm">
        <div class="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-700">
          <dt class="text-slate-500 dark:text-slate-400">Replicate matching</dt>
          <dd class="mt-1 font-medium text-slate-900 dark:text-slate-100">{formatNumber(parameters.replicate_rt_tol, 2)} min / {formatNumber(parameters.replicate_mz_tol, 2)} {formatMode(parameters.replicate_mz_mode)}</dd>
        </div>
        <div class="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-700">
          <dt class="text-slate-500 dark:text-slate-400">Blank subtraction</dt>
          <dd class="mt-1 font-medium text-slate-900 dark:text-slate-100">{formatNumber(parameters.blank_rt_tol, 2)} min / {formatNumber(parameters.blank_mz_tol, 2)} {formatMode(parameters.blank_mz_mode)}</dd>
        </div>
        <div class="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-700">
          <dt class="text-slate-500 dark:text-slate-400">Signal-to-blank threshold</dt>
          <dd class="mt-1 font-medium text-slate-900 dark:text-slate-100">≥ {formatNumber(parameters.signal_to_blank_min, 1)}</dd>
        </div>
        <div class="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-700">
          <dt class="text-slate-500 dark:text-slate-400">Replicate quality bands</dt>
          <dd class="mt-1 font-medium text-slate-900 dark:text-slate-100">High ≤ {formatNumber(parameters.cv_high_max, 1)}% CV, Moderate ≤ {formatNumber(parameters.cv_moderate_max, 1)}% CV</dd>
        </div>
      </dl>
    </section>
  </div>
  ...
  <section class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
    <div class="border-b border-slate-100 px-6 py-5 dark:border-slate-700">
      <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Screened Peaks</h3>
      <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Peak-level QC metrics with blank subtraction and replicate audit trail.</p>
    </div>
    <div class="grid gap-3 border-b border-slate-100 bg-slate-50 px-6 py-4 dark:border-slate-700 dark:bg-slate-700/50 md:grid-cols-6">
      <label class="space-y-1 text-sm">
        <span class="font-medium text-slate-600 dark:text-slate-300">Status</span>
        <select bind:value={statusFilter} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-slate-900 shadow-sm dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100">
      ...
      <label class="space-y-1 text-sm">
        <span class="font-medium text-slate-600 dark:text-slate-300">Polarity</span>
        <select bind:value={polarityFilter} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-slate-900 shadow-sm dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100">
          <option value="all">All</option>
      ...
      <label class="space-y-1 text-sm">
        <span class="font-medium text-slate-600 dark:text-slate-300">Replicate Quality</span>
        <select bind:value={qualityFilter} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-slate-900 shadow-sm dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100">
          <option value="all">All</option>
      ...
      <label class="space-y-1 text-sm">
        <span class="font-medium text-slate-600 dark:text-slate-300">Sample Type</span>
        <select bind:value={sampleFilter} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-slate-900 shadow-sm dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100">
          <option value="all">All</option>
      ...
      <label class="space-y-1 text-sm md:col-span-2">
        <span class="font-medium text-slate-600 dark:text-slate-300">Sort</span>
        <div class="flex gap-2">
          <select bind:value={sortKey} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-slate-900 shadow-sm dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100">
          ...
          <button class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600" onclick={resetFilters}>
            Reset
          </button>
        </div>
      </label>
    </div>
    <div class="flex items-center justify-between border-b border-slate-100 px-6 py-3 text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
          {#each filteredPeaks as peak}
            <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-700/50">
              <td class="px-4 py-3 font-mono text-slate-700 dark:text-slate-300">{formatMaybe(peak.RT_mean, 4)}</td>
              <td class="px-4 py-3 font-mono text-slate-700 dark:text-slate-300">{formatMaybe(peak.MZ_mean, 4)}</td>
              <td class="px-4 py-3 text-slate-900 dark:text-slate-100">{formatNumber(peak.Area_mean, 2)}</td>
              <td class="px-4 py-3">
                <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700 dark:bg-slate-700 dark:text-slate-300">
                  n={peak.ReplicateCount || 2}
                </span>
              </td>
              <td class="px-4 py-3 font-mono text-slate-700 dark:text-slate-300">{formatMaybe(peak.AreaCVPct, 2)}</td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3 font-mono text-slate-700 dark:text-slate-300">{formatMaybe(peak.SignalToBlankRatio, 2)}</td>
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  ...
                  <span class="font-mono text-slate-700 dark:text-slate-300">{formatMaybe(peak.ConfidenceScore, 1)}</span>
                </div>
              </td>
              <td class="px-4 py-3">
                <div class="flex flex-col">
                  <span class="font-semibold capitalize text-slate-900 dark:text-slate-100">{peak.SampleType}</span>
                  <span class="text-xs uppercase text-slate-500 dark:text-slate-400">{peak.Polarity}</span>
                </div>
              </td>
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  <div class="flex flex-col items-center">
                    <div
                      class="h-4 w-4 rounded-full border border-black/10 shadow-sm dark:border-white/10"
                  ...
                  {#if (peak.ReplicateCount || 2) > 2}
                    <span class="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-600 dark:bg-slate-700 dark:text-slate-300">
              ...
              <td class="px-4 py-3">
                <button
                  class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600"
  ...
  >
    ...
    >
      <div class="flex items-start justify-between border-b border-slate-100 px-6 py-5 dark:border-slate-700">
        <div>
          <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">Audit Trail</p>
          <h3 class="mt-2 text-xl font-semibold text-slate-950 dark:text-slate-50">{selectedPeak.Status} • {selectedPeak.SampleType} / {selectedPeak.Polarity}</h3>
        </div>
      ...
      <div class="grid gap-4 border-b border-slate-100 bg-slate-50 px-6 py-4 text-sm dark:border-slate-700 dark:bg-slate-700/50 md:grid-cols-4">
        <div>
          <div class="text-slate-500 dark:text-slate-400">RT / m/z</div>
        ...
        <div>
          <div class="text-slate-500 dark:text-slate-400">Area mean</div>
        ...
        <div>
          <div class="text-slate-500 dark:text-slate-400">Confidence</div>
