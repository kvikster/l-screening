...
<script lang="ts">
    ...
    const steps = [
        ...
        {
            id: "flexibility",
            title: "Гнучкість аналізатора (v0.9.0)",
            short: "Формати · опц. виміри · blank · RSD",
            tone: "blue",
            summary: "Конвеєр стійкий до реальних вхідних даних: інші прилади, відсутні Polarity/Base Peak та чисті blank. Уточнено й термінологію: CV% = RSD, 1 реплікат = 1 хроматограма.",
            deepDive: "1) Формати CSV/TSV/TXT/XLSX/XLS + профіль приладу зіставляє інші заголовки (RetentionTime→RT, PeakArea→Area) і може оголосити Base Peak/Polarity відсутніми. 2) Без Polarity усі рядки — одна група «n/a»; без Polarity і Base Peak — режим RT-only (зіставлення лише за RT, штраф за m/z знятий). 3) Якщо корзини blank немає, Why.BlankState розрізняє: blank_subtracted, blank_clean_here (blank чистий тут) і no_blank_loaded (blank узагалі не завантажено) — обидва «без blank» лишаються Real Compound. 4) Сурогат підказує «ширину корзини» (replicate RT tol): Why.BinWidthSuggestion = MaxAbsRtShift, KStdevRt (k·σ, k=3), RangeRt — рекомендаційно, оператор вводить вручну. 5) CV% позначено як RSD (CV%); ReplicateCount — «Реплікати (хроматограми)», з підсвічуванням походження.",
            input: ["Файли CSV/TSV/TXT/XLSX/XLS", "Профіль приладу (зіставлення колонок)", "Опційні Polarity / Base Peak"],
            action: ["Зіставити заголовки приладу з канонічними полями", "Авто-визначити polarity_available / mz_available", "Класифікувати стан blank (3 стани)", "Порахувати метрики ширини корзини сурогата"],
            output: ["Канонічні рядки незалежно від приладу", "Why.BlankState · Why.BinWidthSuggestion", "Підписи RSD (CV%) та Реплікати (хроматограми)"],
            formula: "polarity_available=false ⇒ group=\"n/a\";  BlankState ∈ {subtracted, clean_here, no_blank}",
            formulaExplanation: "Прапорці polarity_available / mz_available керують групуванням і штрафами. Стан blank явний, а не порожній: чистий blank читається як сильний результат."
        },
        {
            id: "classify",
        ...
        },
        {
            ...
            deepDive: "Жадібна кластеризація: пік із найбільшою площею стає зерном. Із кожного іншого кошика реплікатів вибирається найближчий пік у просторі (RT + m/z). Якщо збіги знайдено щонайменше у 2 кошиках, кластер підтверджується. Алгоритм також підтримує режим RT-only (GC-FID, LC-UV та інші прилади без мас-спектрометра): якщо в наборі даних відсутня колонка «Base Peak», кластеризація та blank subtraction виконуються лише за RT, а штраф confidence score за відсутність m/z не застосовується.",
        ...
        {
            id: "parallel_merge",
            title: "Злиття паралельних проб",
            short: "sample_1 ∩ sample_2",
            tone: "indigo",
            summary: "Підтверджені sample-кластери з різних паралельних проб (sample_1, sample_2 …) зводяться в єдиний рядок. Blank-статус кожної проби вже відомий до злиття.",
            deepDive: "Злиття зважене за кількістю реплікатів. Якщо лише одна проба мала blank match, blank_area_mean агрегується тільки по ній. S/B ratio і статус перераховуються на агрегованому рівні, тому одна шумна проба не псує висновок.",
            input: ["Per-sample кластери зі статусом blank subtraction"],
            action: ["Жадібна кластеризація між пробами (RT + m/z)", "Зважене усереднення (area, RT, mz)", "Агрегація blank_area_mean по джерелах із match", "Перерахунок S/B і confidence_score"],
            output: ["Merged ConfirmedRow з Why.BlankSubtraction.PerSource[]"],
            formula: "S/B_merged = Area_merged / blank_area_mean_weighted",
            formulaExplanation: "Агрегований blank_area_mean рахується зваженим середнім тільки по тих джерелах, що мали blank match. Проба без match не занижує знаменник і не маскує реальний сигнал."
        },
        {
            id: "decision",
            title: "Класифікація",
    ...
    const columns = [
        ...
        { col: "Base Peak", type: "number", desc: "(Опційно) m/z домінуючого іона в мас-спектрі. Відсутність колонки активує режим RT-only (GC-FID, LC-UV).", ex: "195.08" },
    ...
    const outputFields = [
        ...
        { field: "AreaCVPct",          desc: "RSD (відносне стандартне відхилення) площ реплікатів — та сама величина, що CV%. В UI: «RSD (CV%)»." },
        { field: "ReplicateCount",     desc: "Кількість реплікатів. 1 реплікат = 1 хроматограма." },
        { field: "ReplicateQuality",   desc: "High (CV% \u2264 15) / Moderate (\u2264 30) / Low (> 30) — рівень відтворюваності." },
        { field: "SignalToBlankRatio", desc: "S/B ratio для найближчого blank-піка; null, коли віднімати нічого." },
        { field: "Why.BlankState",     desc: "blank_subtracted · blank_clean_here · no_blank_loaded. Останні два лишаються Real Compound." },
        { field: "Why.BinWidthSuggestion", desc: "Для сурогатів: MaxAbsRtShift, KStdevRt (k·σ), RangeRt — рекомендаційна ширина корзини." },
    ...
    const params = [
        ...
        { name: "min_area_difference",    def: "—",    unit: "counts",  used: "(Опційно) абсолютний поріг AreaDiff; Artifact якщо area_sample−area_blank < порогу" },
        { name: "cv_high_max",        def: "15",   unit: "%",       used: "ReplicateQuality = High" },
        { name: "cv_moderate_max",    def: "30",   unit: "%",       used: "ReplicateQuality = Moderate" },
        { name: "mz_available",       def: "true", unit: "bool",    used: "Автовизначається: false для RT-only наборів даних (без колонки Base Peak)" },
    ];
