...
<script lang="ts">
    ...
    const steps = [
        ...
        {
            id: "flexibility",
            title: "Analyzer flexibility (v0.9.0)",
            short: "Formats · optional dims · blank · RSD",
            tone: "blue",
            summary: "The pipeline tolerates real-world inputs: other instruments, missing Polarity/Base Peak, and clean blanks. Terminology is clarified too: CV% = RSD, 1 replicate = 1 chromatogram.",
            deepDive: "1) Formats CSV/TSV/TXT/XLSX/XLS + an instrument profile maps foreign headers (RetentionTime→RT, PeakArea→Area) and may declare Base Peak/Polarity absent. 2) Without Polarity all rows form one \"n/a\" group; without Polarity and Base Peak it runs RT-only (match on RT alone, missing-m/z penalty waived). 3) When no blank bin exists, Why.BlankState distinguishes blank_subtracted, blank_clean_here (a blank exists but is clean here) and no_blank_loaded (no blank at all) — both no-blank cases stay Real Compound. 4) A surrogate suggests the cluster \"bin width\" (replicate RT tol): Why.BinWidthSuggestion = MaxAbsRtShift, KStdevRt (k·σ, k=3), RangeRt — advisory, entered manually. 5) CV% is labeled RSD (CV%); ReplicateCount shown as \"Replicates (chromatograms)\" with provenance highlighting.",
            input: ["CSV/TSV/TXT/XLSX/XLS files", "Instrument profile (column mapping)", "Optional Polarity / Base Peak"],
            action: ["Map instrument headers onto canonical fields", "Auto-detect polarity_available / mz_available", "Classify the blank state (3 states)", "Compute surrogate bin-width metrics"],
            output: ["Canonical rows regardless of instrument", "Why.BlankState · Why.BinWidthSuggestion", "RSD (CV%) and Replicates (chromatograms) labels"],
            formula: "polarity_available=false ⇒ group=\"n/a\";  BlankState ∈ {subtracted, clean_here, no_blank}",
            formulaExplanation: "polarity_available / mz_available flags drive grouping and penalties. The blank state is explicit, not empty: a clean blank reads as a strong result."
        },
        {
            id: "classify",
    ...
    const outputFields = [
        ...
        { field: "AreaCVPct",          desc: "RSD (relative standard deviation) of replicate areas — the same quantity as CV%. Shown in the UI as \"RSD (CV%)\"." },
        { field: "ReplicateCount",     desc: "Number of replicates. 1 replicate = 1 chromatogram." },
        { field: "ReplicateQuality",   desc: "High (CV% \u2264 15) / Moderate (\u2264 30) / Low (> 30) \u2014 reproducibility band." },
        { field: "SignalToBlankRatio", desc: "S/B ratio for the closest blank peak; null when there is nothing to subtract." },
        { field: "Why.BlankState",     desc: "blank_subtracted · blank_clean_here · no_blank_loaded. The last two stay Real Compound." },
        { field: "Why.BinWidthSuggestion", desc: "On surrogates: MaxAbsRtShift, KStdevRt (k·σ), RangeRt — advisory cluster bin width." },
