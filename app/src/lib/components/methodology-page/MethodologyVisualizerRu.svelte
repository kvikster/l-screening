...
<script lang="ts">
    ...
    const steps = [
        ...
        {
            id: "flexibility",
            title: "Гибкость анализатора (v0.9.0)",
            short: "Форматы · опц. измерения · blank · RSD",
            tone: "blue",
            summary: "Конвейер устойчив к реальным входным данным: другие приборы, отсутствующие Polarity/Base Peak и чистые blank. Уточнена и терминология: CV% = RSD, 1 репликат = 1 хроматограмма.",
            deepDive: "1) Форматы CSV/TSV/TXT/XLSX/XLS + профиль прибора сопоставляет чужие заголовки (RetentionTime→RT, PeakArea→Area) и может объявить Base Peak/Polarity отсутствующими. 2) Без Polarity все строки — одна группа «n/a»; без Polarity и Base Peak — режим RT-only (сопоставление только по RT, штраф за m/z снят). 3) Когда корзины blank нет, Why.BlankState различает blank_subtracted, blank_clean_here (blank чист здесь) и no_blank_loaded (blank вообще не загружен) — оба случая «без blank» остаются Real Compound. 4) Суррогат подсказывает «ширину корзины» (replicate RT tol): Why.BinWidthSuggestion = MaxAbsRtShift, KStdevRt (k·σ, k=3), RangeRt — рекомендательно, вводится вручную. 5) CV% обозначен как RSD (CV%); ReplicateCount — «Репликаты (хроматограммы)» с подсветкой происхождения.",
            input: ["Файлы CSV/TSV/TXT/XLSX/XLS", "Профиль прибора (сопоставление колонок)", "Опциональные Polarity / Base Peak"],
            action: ["Сопоставить заголовки прибора с каноническими полями", "Авто-определить polarity_available / mz_available", "Классифицировать состояние blank (3 состояния)", "Вычислить метрики ширины корзины суррогата"],
            output: ["Канонические строки независимо от прибора", "Why.BlankState · Why.BinWidthSuggestion", "Подписи RSD (CV%) и Репликаты (хроматограммы)"],
            formula: "polarity_available=false ⇒ group=\"n/a\";  BlankState ∈ {subtracted, clean_here, no_blank}",
            formulaExplanation: "Флаги polarity_available / mz_available управляют группировкой и штрафами. Состояние blank явное, а не пустое: чистый blank читается как сильный результат."
        },
        {
            id: "classify",
    ...
    const outputFields = [
        ...
        { field: "AreaCVPct",          desc: "RSD (относительное стандартное отклонение) площадей репликатов — та же величина, что CV%. В UI: «RSD (CV%)»." },
        { field: "ReplicateCount",     desc: "Число репликатов. 1 репликат = 1 хроматограмма." },
        { field: "ReplicateQuality",   desc: "High (CV% \u2264 15) / Moderate (\u2264 30) / Low (> 30) — уровень воспроизводимости." },
        { field: "SignalToBlankRatio", desc: "S/B ratio для ближайшего blank-пика; null, когда вычитать нечего." },
        { field: "Why.BlankState",     desc: "blank_subtracted · blank_clean_here · no_blank_loaded. Последние два остаются Real Compound." },
        { field: "Why.BinWidthSuggestion", desc: "Для суррогатов: MaxAbsRtShift, KStdevRt (k·σ), RangeRt — рекомендательная ширина корзины." },
